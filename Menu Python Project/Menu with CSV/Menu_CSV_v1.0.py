...
## Made by: H.Thong Pham ##

#importing python csv library enable to read csv file.
import csv

# Define parallel array to store shop items.
...
# Assign variable to False, indicating data must be loaded before viewing the full record
data_loaded = False
...
# Open CSV file and extract data
    try:
        with open('inventory.csv', mode='r') as file:
            reader = csv.reader(file)  # reads csv file and stores in reader
            for row in reader:  # building the parallel arrays
                sku.append(row[0])
                it_name.append(row[1])
...
def add_data() -> None:
    ...
    while True:
        ...
        if ans == "N": # The loop will exit if the user hits "N"
            break

    # To add the new items to the csv file.
    with open("inventory.csv", mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([sk, it, cat, pr, qua])
...
def main_menu():
    ...
    while True:
        ...
        # if user not entering digit 1 to 4, the program displays error and ask user to reenter.
